...
## 舵轮转向优化仿真
### 新增文件
- **`User_chariot/Simulate/crt_swerve_simulate.cpp/.h`** - 舵轮转向优化仿真代码，用于模拟和验证转向策略。

### 代码调整
- 调整了 CAN_ID 配置，优化通信分组。
- 舵向电机修正，改善转向响应特性。
...
## 遥控器更新与KFS导轨校准
### 代码调整
- 使用新遥控器，重映射 `weapon` 的 `move` 校准。
- KFS 导轨校准调试完成。
- 同步电机模块更新，统一接口。
- 新增抬升基类 `Class_Lift_Base`，为后续抬升机构提供统一框架。
...
## 舵轮单轮调试完成
### 代码调整
- 删去了 `Class_Chassis` 和 `Class_Swerve_Module` 之间重复的参数，消除冗余。
- 优化舵轮控制参数，提升转向平滑度和响应速度。
...
### 代码调整
- 文档更新，完善代码注释和接口说明。
- 继续完成武器夹取代码，写入部分 KFS 代码（参数待定）。

### 后续目标
- 完善参数，当前一些机械结构参数仍然留空，后续完善后需要修改代码中的参数。
...
### 代码调整
- 舵轮编码器测试完成，验证角度读取正确性。
- 添加编码器基类架构 `Class_Encoder_Base`，统一编码器接口。
- CRC_Lib 测试成功，校验功能正常。
